...
import {
  ...
  BarChart3, CircleCheckBig, PartyPopper, CircleX, KeyRound
} from 'lucide-react';
    ...
    <div style={{
      ...
      {loading ? (
        ...
        <div key={n.id} onClick={() => handleClick(n)} style={{
          ...
          cursor: (n.relatedId && n.type !== 'system') ? 'pointer' : (n.isRead ? 'default' : 'pointer'),
          transition:'background .15s',
        }}>
          <div style={{
            flexShrink:0, marginTop:2, width:32, height:32, borderRadius:10,
          ...
          <div style={{ display:'flex', gap:2, flexShrink:0 }}>
            {!n.isRead && (
              <button className="btn btn-ghost btn-sm btn-icon" onClick={e => { e.stopPropagation(); handleRead(n.id); }}>
                <Check size={12} />
              </button>
            )}
            <button className="btn btn-ghost btn-sm btn-icon" style={{ color:'var(--danger)' }} onClick={e => handleDelete(n.id, e)}>
              <Trash2 size={12} />
            </button>
          </div>
        </div>
...
export default function Header({ title, subtitle, actions }) {
  ...
  return (
    <header className="header" style={{
      ...
      {/* Title — để lại chỗ trống bên trái trên mobile (hamburger fixed) */}
      ...
      <div className="header-actions" style={{ flexWrap:'wrap', gap:6 }}>
        ...
        {/* Chuông — ẩn trên mobile, chỉ hiện desktop */}
        <div ref={ref} style={{ position:'relative' }} className="header-bell">
